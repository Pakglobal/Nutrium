...
//     // No need to show loading state for this quick operation
...
//       SetWaterIntakeDetails(payload).then(() => {
//         // Only update data from server in background
//         getData();
//         GetWaterIntakeDetails(token, id).then(updatedData => {
//           setWaterIntake(updatedData);
//         }).catch(error => {
//           console.error('Get water intake details error:', error);
//         });
//       }).catch(error => {
//         console.error('Add water error:', error);
//       });
...
const handleAddWater = (amount) => {
  ...
  // Update local water intake state immediately for progress bar
  setLocalIntake(prev => prev + mlAmount);

  // Perform API calls in the background without blocking UI
  const currentDate = new Date();
  const time = `${currentDate.getHours().toString().padStart(2, '0')}:${currentDate.getMinutes().toString().padStart(2, '0')}`;
  ...
  // Fire and forget - don't block UI with these API calls
  setTimeout(() => {
    SetWaterIntakeDetails(payload)
      ...
      .then(updatedData => {
        ...
        // Only update background data if needed
        const shouldUpdateBackground = Math.random() > 0.7; // Only update sometimes to reduce calls
