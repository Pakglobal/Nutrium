...
//   const getStorageKey = (key) => `${key}_${id}`;

//   useEffect(() => {
//     const resetIfUserChanged = async () => {
//       const prevUserId = prevUserIdRef.current;

//       if (prevUserId && prevUserId !== id) {
//         await AsyncStorage.setItem(getStorageKey('sevenL'), '0');
//         await AsyncStorage.setItem(getStorageKey('seventeenL'), '0');
//         await AsyncStorage.setItem(getStorageKey('lastHydrationDate'), new Date().toDateString());
//         setSevenL(0);
//         setSevenTeenL(0);
...
//           await AsyncStorage.setItem(getStorageKey('sevenL'), '0');
//           await AsyncStorage.setItem(getStorageKey('seventeenL'), '0');
//           await AsyncStorage.setItem(getStorageKey('lastHydrationDate'), today);
//           setSevenL(0);
//           setSevenTeenL(0);
//           setCurrentProgress(0);
//           widthAnimation.setValue(0);
//         } else {
//           const savedSevenL = await AsyncStorage.getItem(getStorageKey('sevenL'));
//           const savedSeventeenL = await AsyncStorage.getItem(getStorageKey('seventeenL'));
//           setSevenL(parseFloat(savedSevenL) || 0);
//           setSevenTeenL(parseFloat(savedSeventeenL) || 0);
//         }
//       } catch (e) {
//         console.error('Error loading local water data:', e);
//       }
//     };
...
//   useEffect(() => {
//     let base = 0;
//     if (waterData?.waterIntakes?.length > 0) {
//       base = waterData.waterIntakes.reduce(
//         (sum, entry) => sum + (entry?.amount || 0) / 1000,
//         0
//       );
//     }
//     setCurrentProgress(base + sevenL + seventeenL);
...
//       const time = `${currentDate.getHours().toString().padStart(2, '0')}:${currentDate
//         .getMinutes()
//         .toString()
//         .padStart(2, '0')}`;
...
//                 {`${currentProgress.toFixed(1)}L / ${totalGoal}L`}
//               </Text>
//             </View>
      ...
      <View style={styles.waterContainer}>
        <View style={styles.topContainer}>
          ...
          <View>
            <View style={styles.showIntake}>
              <Text style={styles.intakeTxt}>Current intake</Text>
              <Text style={styles.intakeTxt}>{`${currentProgress.toFixed(1)}L / ${totalGoal}L`}</Text>
            </View>
