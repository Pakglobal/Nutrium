...
const MessageComponent = ({
  ...
  const handleSendMessage = async (isFile = false) => {
    ...
    try {
      ...
      const tempMessage = {
        ...
        createdAt: now,
        seen: false,
        isTemp: false, // Changed from !!fileUrl to always false once we have actual data
      };
      ...
      if (isFile && DocumentPicker.isCancel(err)) {
        console.warn('User cancelled file picker');
      } else {
  ...
  const renderMessage = ({ item }) => {
    ...
    const fileUrl = item?.file || item?.fileUrl;
    const time = formatTime(item?.createdAt || item?.timestamp);
    const isUploading = item.isTemp || uploadingFiles[item._id] || uploadingFiles[item.tempId];
    const isSeen = item?.seen;
    const isTemp = item.isTemp || (!item._id && item.tempId);
