...
import { challengeAcceptAndRejectedApi, getAllChallengePendingRequest } from '../../../Apis/ClientApis/ChallengesApi';
...
const JoinRequestScreen = () => {
        ...
        return (
            <View style={styles.card}>
                ...
                <Text style={styles.detail}>
                    Start: {moment(item.startDate).format('MMM DD, YYYY')} - End: {moment(item.endDate).format('MMM DD, YYYY')}
                </Text>
                <Text style={styles.detail}>
    ...
    const aceeptAndRejectAnctionHandle = async (id, action) => {
        try {
            setLoading(true);
            const response = await challengeAcceptAndRejectedApi(userInfo?.token, id, userInfo?.userData?._id, action)
            if (response?.success) {
